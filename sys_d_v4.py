...
def get_comprehensive_graphics_info():
    ...
    if platform.system() == "Windows":
        try:
            output = run_cmd(
                "wmic path win32_videocontroller get name, adapterram, driverversion, videoprocessor, videomodedescription /format:table")
        ...
        try:
            ...
            if output and "No Instance(s) Available" not in output:
                ...
                for line in lines:
                    if line.strip():
                        gpu_info.append({
                            "Graphics Card": line.strip(),
                            "Status": "Detected (Limited Info)"
                        })
        except:
            pass
...
def get_advanced_network_properties():
    ...
    try:
        ...
        detailed_info.extend([
            ...
            {"Property": "Network Scan Time", "Value": datetime.now().strftime('%H:%M:%S')}
        ])
...
def generate_html_report():
    ...
    <body>
        <div class="container">
            <div class="header">
                <h1>Advanced System Comprehensive Report</h1>
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            print_table(get_network_analysis(), tablefmt="simple")
