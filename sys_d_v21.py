...
# -------------------------------------------------------------------
#  NEW FEATURES ADDED: VULNERABILITY SCANNING, ENCRYPTION STATUS, 
...
def get_vulnerability_assessment():
    ...
    print("\n\t", end='')
    print_status("Performing vulnerability assessment...", "SCAN")
    vulnerabilities = []
    ...
    try:
        if platform.system() == "Windows":
            ...
            try:
                ...
                if '0x0' in uac_output:
                    vulnerabilities.append({
                        ...
                        "Severity": "HIGH",
                        "Description": "User Account Control is disabled, making system vulnerable to privilege escalation",
            ...
            try:
                update_output = run_cmd('powershell "Get-WindowsUpdate"')
                if "No updates are available" not in update_output and "Installed" not in update_output:
                    vulnerabilities.append({
                        "Vulnerability": "Outdated Windows",
                        "Severity": "HIGH",
                        "Description": "Windows updates are not installed, leaving system vulnerable to known exploits",
            ...
            try:
                smb_output = run_cmd('powershell "Get-WindowsOptionalFeature -Online -FeatureName SMB1Protocol"')
                if "Enabled" in smb_output:
                    vulnerabilities.append({
                        "Vulnerability": "SMBv1 Enabled",
                        "Severity": "CRITICAL",
                        "Description": "SMBv1 protocol is enabled (vulnerable to WannaCry, EternalBlue)",
                        "Remediation": "Disable SMBv1: Control Panel > Programs > Turn Windows features on/off > Uncheck SMB 1.0"
                    })
            except:
                pass
            ...
            try:
                rdp_output = run_cmd('reg query "HKLM\\SYSTEM\\CurrentControlSet\\Control\\Terminal Server" /v fDenyTSConnections')
                if '0x0' in rdp_output:
                    vulnerabilities.append({
                        "Vulnerability": "RDP Enabled",
                        "Severity": "MEDIUM",
                        "Description": "Remote Desktop Protocol is enabled, potential attack surface",
                        "Remediation": "Disable RDP if not needed: System Properties > Remote > Don't allow remote connections"
                    })
            except:
                pass

            # Check AutoRun for all drives
            try:
                autorun_output = run_cmd('reg query "HKLM\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Policies\\Explorer" /v NoDriveTypeAutoRun')
                if not autorun_output or "0x0" in autorun_output:
                    vulnerabilities.append({
                        "Vulnerability": "AutoRun Enabled",
                        "Severity": "MEDIUM",
                        "Description": "AutoRun is enabled for removable drives, vulnerable to malware",
            ...
            try:
                ps_policy = run_cmd('powershell "Get-ExecutionPolicy"')
                if "Unrestricted" in ps_policy or "Bypass" in ps_policy:
                    vulnerabilities.append({
                        "Vulnerability": "PowerShell Unrestricted",
                        "Severity": "HIGH",
                        "Description": "PowerShell execution policy allows unsigned scripts",
                        "Remediation": "Set execution policy to RemoteSigned: Set-ExecutionPolicy RemoteSigned"
                    })
            except:
                pass
        ...
        vulnerabilities.append({
            ...
            "Severity": "MEDIUM",
            "Description": "System may have weak password policies",
            "Remediation": "Enforce strong password policy: minimum 12 characters, complexity requirements"
        })
        ...
        try:
            ...
            if open_ports:
                vulnerabilities.append({
                    ...
                    "Remediation": "Close unnecessary ports via Windows Firewall"
                })
        except:
            pass

        # Check .NET Framework versions (older versions vulnerable)
        try:
            dotnet_output = run_cmd('reg query "HKLM\\SOFTWARE\\Microsoft\\NET Framework Setup\\NDP" /s')
            if "v4." not in dotnet_output:
                vulnerabilities.append({
                    "Vulnerability": "Outdated .NET Framework",
                    "Severity": "MEDIUM",
                    "Description": ".NET Framework 4.x not detected, older versions may have vulnerabilities",
                    "Remediation": "Install latest .NET Framework from Microsoft website"
                })
        except:
            pass

        print("\t", end='')
        print_status(f"Vulnerability assessment completed: {len(vulnerabilities)} issues found", "SUCCESS")

    except Exception as e:
        print_status(f"Vulnerability assessment failed: {str(e)}", "ERROR")

    return vulnerabilities


def get_encryption_security_status():
    """Check encryption and security features status"""
    print("\n\t", end='')
    print_status("Checking encryption and security status...", "SCAN")
    encryption_status = []
    ...
    try:
        if platform.system() == "Windows":
            ...
            try:
                ...
                if bitlocker_output and "Conversion Status" in bitlocker_output:
                    lines = bitlocker_output.split('\n')
                    for line in lines:
                        if "Conversion Status" in line:
                            drive = line.split()[0]
                            status = line.split(":")[1].strip()
                            protection = "ENCRYPTED" if "Encrypted" in status else "UNENCRYPTED"
                            encryption_status.append({
                                "Drive": drive,
                                "Encryption": "BitLocker",
                                "Status": protection,
                                "Protection Status": status,
                                "Risk": "LOW" if "Encrypted" in status else "HIGH"
                            })
                else:
                    encryption_status.append({
                        "Drive": "All Drives",
                        "Encryption": "BitLocker",
                        "Status": "NOT_ACTIVE",
                        "Protection Status": "Drive encryption not enabled",
                        "Risk": "HIGH"
                    })
            except:
                encryption_status.append({
                    "Drive": "BitLocker Check",
                    "Encryption": "BitLocker",
                    "Status": "CHECK_FAILED",
                    "Protection Status": "Unable to verify BitLocker status",
                    "Risk": "UNKNOWN"
                })
            ...
            try:
                ...
                if tpm_output and "TpmPresent" in tpm_output:
                    if "True" in tpm_output:
                        encryption_status.append({
                            "Drive": "TPM Chip",
                            "Encryption": "Hardware Security",
                            "Status": "PRESENT",
                            "Protection Status": "Trusted Platform Module available",
                            "Risk": "LOW"
                        })
                    else:
                        encryption_status.append({
                            ...
                            "Encryption": "Hardware Security",
                            "Status": "NOT_PRESENT",
                            "Protection Status": "No hardware security module",
                            "Risk": "MEDIUM"
                        })
            except:
                pass
            ...
            try:
                secureboot_output = run_cmd('powershell "Confirm-SecureBootUEFI"')
                if secureboot_output and "True" in secureboot_output:
                    encryption_status.append({
                        "Drive": "UEFI/Firmware",
                        "Encryption": "Secure Boot",
                        "Status": "ENABLED",
                        "Protection Status": "Secure Boot is active",
                        "Risk": "LOW"
                    })
                else:
                    encryption_status.append({
                        ...
                        "Encryption": "Secure Boot",
                        "Status": "DISABLED",
                        "Protection Status": "Secure Boot is not enabled",
                        "Risk": "MEDIUM"
                    })
            except:
                pass

            # Check Device Encryption (Windows 10/11 Home)
            try:
                ...
                if device_encryption and "On" in device_encryption:
                    encryption_status.append({
                        ...
                        "Status": "ENABLED",
                        "Protection Status": "Device encryption is active",
                        "Risk": "LOW"
                    })
            except:
                pass

            # Check for encrypted files (EFS)
            try:
                efs_output = run_cmd('cipher /u /n')
                if efs_output and "encrypted file(s)" in efs_output.lower():
                    encryption_status.append({
                        "Drive": "User Files",
                        "Encryption": "EFS (Encrypting File System)",
                        "Status": "IN_USE",
                        "Protection Status": "Some files are encrypted with EFS",
                        "Risk": "LOW"
                    })
            except:
                pass

        # Check for common encryption software
        encryption_software = [
            "veracrypt", "truecrypt", "axcrypt", "7-zip", "winrar", "bitlocker", "cryptomator"
        ]
        
        installed_encryption = []
        for software in encryption_software:
            try:
                if platform.system() == "Windows":
                    output = run_cmd(f'where {software}')
                    if output and "Could not find" not in output:
                        installed_encryption.append(software.upper())
            except:
                continue
        
        if installed_encryption:
            encryption_status.append({
                "Drive": "Third-party",
                "Encryption": ", ".join(installed_encryption),
                "Status": "INSTALLED",
                "Protection Status": "Encryption software detected",
                "Risk": "LOW"
            })

        print("\t", end='')
        print_status(f"Encryption status check completed: {len(encryption_status)} items analyzed", "SUCCESS")

    except Exception as e:
        print_status(f"Encryption status check failed: {str(e)}", "ERROR")

    return encryption_status


def extract_browser_data():
    """Extract browser data (history, bookmarks, cookies)"""
    print("\n\t", end='')
    print_status("Extracting browser data...", "SCAN")
    browser_data = []

    try:
        # Chrome browser data
        chrome_paths = {
            "History": os.path.join(os.environ.get('LOCALAPPDATA', ''), "Google\\Chrome\\User Data\\Default\\History"),
            "Bookmarks": os.path.join(os.environ.get('LOCALAPPDATA', ''), "Google\\Chrome\\User Data\\Default\\Bookmarks"),
            "Cookies": os.path.join(os.environ.get('LOCALAPPDATA', ''), "Google\\Chrome\\User Data\\Default\\Cookies"),
            "Login Data": os.path.join(os.environ.get('LOCALAPPDATA', ''), "Google\\Chrome\\User Data\\Default\\Login Data")
        }

        chrome_found = False
        for data_type, path in chrome_paths.items():
        ...
        if chrome_found:
            print("\t\t", end='')
            print_status("Chrome browser data detected", "DATA")
        ...
        for profile in firefox_profiles[:2]:  # Check first 2 profiles
            places_db = os.path.join(profile, "places.sqlite")
            if os.path.exists(places_db):
                browser_data.append({
                    "Browser": "Mozilla Firefox",
                    "Data Type": "History/Bookmarks",
                    "Path": places_db,
                    "Status": "FOUND",
                    "Size": f"{os.path.getsize(places_db) / 1024:.1f} KB"
                })

        # Edge browser data
        edge_paths = {
            "History": os.path.join(os.environ.get('LOCALAPPDATA', ''), "Microsoft\\Edge\\User Data\\Default\\History"),
            "Bookmarks": os.path.join(os.environ.get('LOCALAPPDATA', ''), "Microsoft\\Edge\\User Data\\Default\\Bookmarks"),
        }

        edge_found = False
        for data_type, path in edge_paths.items():
            if os.path.exists(path):
                edge_found = True
                browser_data.append({
        ...
        if edge_found:
            print("\t\t", end='')
            print_status("Edge browser data detected", "DATA")

        # Check for saved passwords (generic detection)
        browsers_with_passwords = []
        if chrome_found:
            browsers_with_passwords.append("Chrome")
        if edge_found:
            browsers_with_passwords.append("Edge")
        
        if browsers_with_passwords:
            browser_data.append({
                "Browser": ", ".join(browsers_with_passwords),
                "Data Type": "Saved Passwords",
                "Path": "Browser Password Storage",
                "Status": "POTENTIALLY_SAVED",
                "Size": "ENCRYPTED"
            })

        # Count cookies (estimated)
        total_cookies = 0
        for item in browser_data:
            if "Cookies" in item["Data Type"]:
                total_cookies += int(item["Size"].split()[0])
        
        if total_cookies > 0:
            browser_data.append({
                "Browser": "All Browsers",
                "Data Type": "Total Cookies",
                "Path": "Browser Storage",
                "Status": f"APPROX_{total_cookies}_COOKIES",
                "Size": "Various"
            })
        ...
        print("\t", end='')
        print_status(f"Browser data extraction completed: {len([b for b in browser_data if b['Status'] == 'FOUND'])} items found", "SUCCESS")

    except Exception as e:
        print_status(f"Browser data extraction failed: {str(e)}", "ERROR")
        browser_data.append({
            "Browser": "ERROR",
            "Data Type": f"Extraction failed: {str(e)}",
            "Path": "N/A",
            "Status": "ERROR",
            "Size": "N/A"
        })
...
def detect_remote_access_tools():
    """Detect remote access and administration tools"""
    print("\n\t", end='')
    print_status("Detecting remote access tools...", "SCAN")
    remote_tools = []

    try:
        # Common remote access tools process names
        remote_tool_indicators = {
            "RDP": ["mstsc", "rdp", "termservice", "remote desktop"],
            "TeamViewer": ["teamviewer", "tv_w32", "tv_x64"],
            "AnyDesk": ["anydesk", "anydesk.exe"],
            "VNC": ["vnc", "tightvnc", "ultravnc", "realvnc", "winvnc"],
            "Chrome Remote": ["chrome", "remotedesktop", "chromoting"],
            "SSH": ["ssh", "openssh", "putty", "kitty", "mobaxterm"],
            "LogMeIn": ["logmein", "lmi", "lmig"],
            "Splashtop": ["splashtop", "spstreamer"],
            "Zoom": ["zoom", "zoommtg"],
            "Microsoft Teams": ["teams", "msteams"],
            "Discord": ["discord"],
            "Skype": ["skype"],
            "Radmin": ["radmin", "rserver"],
            "Ammyy Admin": ["ammyy", "aa_v3"],
            "DWService": ["dwservice", "dws"],
            "Supremo": ["supremo", "supcontrol"],
            "AeroAdmin": ["aeroadmin"],
            "Remote Utilities": ["rutserv", "rutview"],
            "ScreenConnect": ["screenconnect", "connectwise"],
            "GoToMyPC": ["gotomypc", "gtpc"],
            "Parallels Access": ["racc", "parallels"]
        }
        ...
        for proc in psutil.process_iter(['name', 'exe']):
            try:
                ...
                for tool, indicators in remote_tool_indicators.items():
                    for indicator in indicators:
                        if indicator in proc_name or (proc_exe and indicator in proc_exe):
                            if tool not in detected_tools:
                                ...
                                remote_tools.append({
                                    ...
                                    "Status": "RUNNING",
                                    "Risk Level": "HIGH" if tool in ["TeamViewer", "AnyDesk", "Ammyy Admin"] else "MEDIUM",
                                    "Detection Method": "Process Analysis"
                                })
            except (psutil.NoSuchProcess, psutil.AccessDenied):
                continue
        ...
        if platform.system() == "Windows":
            try:
                installed_output = run_cmd('wmic product get name /value')
                for line in installed_output.split('\n'):
                    if '=' in line:
                        program = line.split('=')[1].strip().lower()
                        for tool, indicators in remote_tool_indicators.items():
                            for indicator in indicators:
                                if indicator in program and tool not in detected_tools:
                                    detected_tools.append(tool)
                                    remote_tools.append({
                                        "Tool Name": tool,
                                        "Process": "Not Running",
                                        "Status": "INSTALLED",
                                        "Risk Level": "MEDIUM",
                                        "Detection Method": "Installed Programs"
                                    })
            except:
                pass
            ...
            # Check services
            try:
                services_output = run_cmd('sc query type= service state= all')
                for tool, indicators in remote_tool_indicators.items():
                    for indicator in indicators:
                        if indicator in services_output.lower() and tool not in detected_tools:
                            detected_tools.append(tool)
                            remote_tools.append({
                                "Tool Name": tool,
                                "Process": "Service",
                                ...
                                "Risk Level": "MEDIUM",
                                "Detection Method": "Service Detection"
                            })
            except:
                pass

            # Check registry entries for auto-start
            registry_locations = [
            ...
            for location in registry_locations:
                try:
                    reg_output = run_cmd(f'reg query "{location}"')
                    for tool, indicators in remote_tool_indicators.items():
                        for indicator in indicators:
                            if indicator in reg_output.lower() and tool not in detected_tools:
                                detected_tools.append(tool)
                                remote_tools.append({
                                    "Tool Name": tool,
                                    "Process": "Registry AutoStart",
                                    "Status": "AUTO_START_ENABLED",
                                    "Risk Level": "HIGH",
                                    "Detection Method": "Registry Analysis"
                                })
                except:
                    continue

        # Check for RDP specific configurations
        if platform.system() == "Windows":
            try:
                rdp_reg = run_cmd('reg query "HKLM\\SYSTEM\\CurrentControlSet\\Control\\Terminal Server" /v fDenyTSConnections')
                if '0x0' in rdp_reg:
                    if "RDP" not in detected_tools:
                        remote_tools.append({
                            "Tool Name": "RDP",
                            "Process": "System Service",
                            "Status": "ENABLED",
                            "Risk Level": "MEDIUM",
                            "Detection Method": "Registry Setting"
                        })
            except:
                pass

        # Network connection analysis for remote tools
        try:
            for conn in psutil.net_connections(kind='inet'):
                if conn.raddr:
                    ...
                    if port in remote_ports and remote_ports[port] not in detected_tools:
                        remote_tools.append({
                            "Tool Name": remote_ports[port],
                            "Process": f"Port {port} Connection",
                            "Status": "NETWORK_ACTIVITY",
                            "Risk Level": "HIGH",
                            "Detection Method": "Network Analysis"
                        })
        except:
            pass
        ...
        if not remote_tools:
            remote_tools.append({
                "Tool Name": "No Remote Tools",
                "Process": "N/A",
                "Status": "NOT_DETECTED",
                "Risk Level": "LOW",
                "Detection Method": "Clean System"
            })

        print("\t", end='')
        print_status(f"Remote access detection completed: {len([r for r in remote_tools if r['Tool Name'] != 'No Remote Tools'])} tools detected", "SUCCESS")

    except Exception as e:
        print_status(f"Remote access detection failed: {str(e)}", "ERROR")
        remote_tools.append({
            "Tool Name": "Detection Error",
...
def generate_html_report():
    ...
    sections_data = {
        ...
        "VULNERABILITY ASSESSMENT": get_vulnerability_assessment(),
        "ENCRYPTION & SECURITY STATUS": get_encryption_security_status(),
        "BROWSER DATA ANALYSIS": extract_browser_data(),
        "REMOTE ACCESS DETECTION": detect_remote_access_tools(),
    ...
    vulnerabilities = sections_data.get("VULNERABILITY ASSESSMENT", [])
            ...
            <div class="section">
                ...
                <div class="security-summary">
                    <div class="security-card critical">
                        <div class="security-count">{critical_count}</div>
                        <div class="security-label">CRITICAL</div>
                    </div>
                    <div class="security-card high">
                        <div class="security-count">{high_count}</div>
                        <div class="security-label">HIGH</div>
                    </div>
                    <div class="security-card medium">
                        <div class="security-count">{medium_count}</div>
                        <div class="security-label">MEDIUM</div>
                    </div>
                    <div class="security-card low">
                        <div class="security-count">{low_count}</div>
                        <div class="security-label">LOW</div>
                    </div>
                </div>
            </div>
    """
    ...
    scrolling_sections = {
        ...
        "VULNERABILITY ASSESSMENT": "wide-table",
        "ENCRYPTION & SECURITY STATUS": "wide-table",
        "BROWSER DATA ANALYSIS": "medium-table",
        "REMOTE ACCESS DETECTION": "wide-table",
    ...
    for section_name, data in sections_data.items():
        ...
        if data:
            ...
            elif section_name == "VULNERABILITY ASSESSMENT":
                # Vulnerability cards in grid layout
                html_content += '<div class="vulnerability-grid">'
                for vuln in data:
                    severity_class = vuln.get("Severity", "MEDIUM").lower()
                    severity_badge = f"severity-{severity_class}-badge"
                    html_content += f'''
                    <div class="vulnerability-card {severity_class}">
                        <h3>{vuln.get("Vulnerability", "Unknown")}</h3>
                        <div class="vulnerability-severity {severity_badge}">
                            {vuln.get("Severity", "MEDIUM")}
                        </div>
                        <p>{vuln.get("Description", "No description")}</p>
                        <div class="remediation">
                            <strong>Remediation:</strong> {vuln.get("Remediation", "No remediation provided")}
                        </div>
                    </div>
                    '''
                html_content += '</div>'
                        ...
                        for i, cell in enumerate(row):
                            ...
                            if headers[i] == "Severity":
                                if "CRITICAL" in cell_str:
                                    row_html += f'<td class="severity-critical">{cell}</td>'
                                elif "HIGH" in cell_str:
                                    row_html += f'<td class="severity-high">{cell}</td>'
                                elif "MEDIUM" in cell_str:
                                    row_html += f'<td class="severity-medium">{cell}</td>'
                                elif "LOW" in cell_str:
                                    row_html += f'<td class="severity-low">{cell}</td>'
                                else:
                                    row_html += f'<td>{cell}</td>'
                            
                            # Risk level highlighting
                            elif headers[i] in ["Risk", "Risk Level"]:
                                if "HIGH" in cell_str or "CRITICAL" in cell_str:
                                    row_html += f'<td class="risk-high">{cell}</td>'
                                elif "MEDIUM" in cell_str:
                                    row_html += f'<td class="risk-medium">{cell}</td>'
                                elif "LOW" in cell_str:
                                    row_html += f'<td class="risk-low">{cell}</td>'
                                else:
                                    row_html += f'<td>{cell}</td>'
                            
                            # Status highlighting
                            elif headers[i] in ["Status", "Account Active", "Health", "State", "Protection Status"]:
                                if any(x in cell_str.upper() for x in 
                                       ["ACTIVE", "YES", "HEALTHY", "CONNECTED", "RUNNING", "ENABLED", "ENCRYPTED", "PRESENT"]):
                                    row_html += f'<td class="status-active">{cell}</td>'
                                elif any(x in cell_str.upper() for x in 
                                         ["INACTIVE", "NO", "ERROR", "DISCONNECTED", "STOPPED", "DISABLED", "UNENCRYPTED", "NOT_PRESENT"]):
                                    row_html += f'<td class="status-inactive">{cell}</td>'
                                elif "CRITICAL" in cell_str.upper():
                                    row_html += f'<td class="status-critical">{cell}</td>'
                                elif "WARNING" in cell_str.upper() or "MEDIUM" in cell_str.upper():
                                    row_html += f'<td class="status-warning">{cell}</td>'
                                elif "NOTICE" in cell_str.upper() or "LOW" in cell_str.upper():
                            ...
                            # Browser data highlighting
                            elif section_name == "BROWSER DATA ANALYSIS" and headers[i] == "Status":
                                if "FOUND" in cell_str:
                                    row_html += f'<td class="status-active">{cell}</td>'
                                elif "POTENTIALLY" in cell_str:
                                    row_html += f'<td class="status-warning">{cell}</td>'
                                elif "NOT_DETECTED" in cell_str:
                                    row_html += f'<td class="status-inactive">{cell}</td>'
                                else:
                                    row_html += f'<td>{cell}</td>'
    ...
    html_content += """
        ...
        <script>
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                const highRiskElements = document.querySelectorAll('.risk-high');
                highRiskElements.forEach(el => {
                    setInterval(() => {
                        el.style.textShadow = el.style.textShadow === '0 0 10px #ff0000' 
                            ? '0 0 20px #ff0000' 
                            : '0 0 10px #ff0000';
                    }, 1000);
                });
            ...
            const criticalCards = document.querySelectorAll('.vulnerability-card.critical');
            criticalCards.forEach(card => {
                setInterval(() => {
                    card.style.boxShadow = card.style.boxShadow === '0 0 20px rgba(255, 0, 0, 0.3)'
...
if __name__ == "__main__":
    ...
    try:
        ...
        sections = [
            ...
            "SYSTEM PERFORMANCE", "VULNERABILITY ASSESSMENT", "ENCRYPTION & SECURITY STATUS",
            "BROWSER DATA ANALYSIS", "REMOTE ACCESS DETECTION", "TASK MANAGER"
        ]
