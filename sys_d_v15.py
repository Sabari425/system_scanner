...
# -------------------------------------------------------------------
#  CONSOLE COLORS - JARVIS CYAN THEME
# -------------------------------------------------------------------
class Colors:
    # Cyan color variants
    CYAN = '\033[96m'
    ...
    DARK_CYAN = '\033[36m'
    LIGHT_CYAN = '\033[96;2m'
    NEON_CYAN = '\033[38;5;51m'
    ELECTRIC_CYAN = '\033[38;5;87m'
    AQUA_CYAN = '\033[38;5;122m'
    ICE_CYAN = '\033[38;5;159m'
    STEEL_CYAN = '\033[38;5;116m'
    CYBER_CYAN = '\033[38;5;123m'
    
    # Text effects with cyan
    BOLD_CYAN = '\033[1;96m'
    BLINK_CYAN = '\033[5;96m'
    UNDERLINE_CYAN = '\033[4;96m'
    ITALIC_CYAN = '\033[3;96m'
    
    # Status colors in cyan theme
    SUCCESS_CYAN = '\033[38;5;85m'  # Bright cyan-green
    WARNING_CYAN = '\033[38;5;215m' # Cyan-orange
    ERROR_CYAN = '\033[38;5;203m'   # Cyan-red
    INFO_CYAN = '\033[38;5;117m'    # Light cyan
    SCAN_CYAN = '\033[38;5;123m'    # Cyber cyan
    DATA_CYAN = '\033[38;5;159m'    # Ice cyan
    SYSTEM_CYAN = '\033[38;5;116m'  # Steel cyan
    
    # Backgrounds
    BG_DARK_CYAN = '\033[48;5;23m'
    BG_CYAN = '\033[48;5;36m'
    BG_LIGHT_CYAN = '\033[48;5;87m'
    ...
    # Gold accents (for JARVIS style)
    GOLD = '\033[38;5;220m'
    BRIGHT_GOLD = '\033[38;5;229m'
    
    # Silver accents
    SILVER = '\033[38;5;255m'
    METALLIC_SILVER = '\033[38;5;188m'

def print_colored(text, color=Colors.CYAN, end="\n"):
    print(f"{color}{text}{Colors.RESET}", end=end)


def print_status(message, status="INFO"):
    status_colors = {
        "INFO": Colors.INFO_CYAN,
        "SUCCESS": Colors.SUCCESS_CYAN,
        "WARNING": Colors.WARNING_CYAN,
        "ERROR": Colors.ERROR_CYAN,
        "SCAN": Colors.SCAN_CYAN,
        "DATA": Colors.DATA_CYAN,
        "SYSTEM": Colors.SYSTEM_CYAN
    }
...
def progress_bar(iteration, total, prefix='', suffix='', length=50, fill='█'):
    ...
    if float(percent) < 30:
        bar_color = Colors.ERROR_CYAN
    elif float(percent) < 70:
        bar_color = Colors.WARNING_CYAN
    else:
        bar_color = Colors.SUCCESS_CYAN

    print_colored(f'\r{prefix} |{bar_color}{bar}{Colors.RESET}| {percent}%   -   {suffix}', Colors.CYAN, end='')
    if iteration == total:
        print()
...
{Colors.RESET}
{Colors.CYBER_CYAN}
                    SYSTEM DIAGNOSTICS SCAN v3.0
                    {Colors.GOLD}◆ SABARI425 ORGANIZATION ◆{Colors.RESET}
"""
    print(banner)

def simulate_scan_step(step_name, duration=1, steps=20):
    """JARVIS-style scanning animation"""
    print("\n\t", end='')
    print_status(f"Analyzing: {step_name}", "SCAN")
    for i in range(steps + 1):
        progress_bar(i, steps, prefix='\t\tProgress:', suffix=step_name, length=60)
        time.sleep(.02)
    print("\t\t", end='')
    print_status(f"Analysis complete: {step_name}", "SUCCESS")
...
def get_health_color(score=None):
    ...
    if score >= 80:
        ...
        return "linear-gradient(135deg, #ffff00 0%, #ff8800 100%)"
    else:
        return "linear-gradient(135deg, #ff4444 0%, #cc0000 100%)"


# -------------------------------------------------------------------
#  HTML REPORT GENERATION - JARVIS CYAN THEME
# -------------------------------------------------------------------
def generate_html_report():
    print_banner()
    print_status("Initializing JARVIS diagnostic protocols...", "SCAN")

    # JARVIS-style scanning steps
    scan_steps = [
        "System Hardware & BIOS Analysis",
        "Memory & Storage Diagnostics", 
        "Network Interfaces & Security",
        "User Accounts & Access Control",
        "Software Inventory & Drivers",
    ...
    html_path = os.path.join(downloads, f"JARVIS_SCAN_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html")

    # Gather all data
    print("\n\n\n")
    print_status("Compiling system diagnostics data...", "SCAN")
    ...
    <head>
        ...
        <title>JARVIS SYSTEM DIAGNOSTICS - SABARI425</title>
        <style>
            ...
            * {{
            ...
            body {{
                ...
                background: linear-gradient(135deg, #0a0a1a 0%, #001122 50%, #0a1a2a 100%);
                min-height: 100vh;
                margin: 0;
                padding: 0;
                color: #00ffff;
            ...
            .jarvis-bg {{
                ...
                background: 
                    ...
                    linear-gradient(45deg, rgba(0, 20, 40, 0.3) 0%, transparent 50%);
                pointer-events: none;
                z-index: -1;
            }}
            ...
            .header {{
                background: linear-gradient(135deg, rgba(0, 40, 80, 0.9) 0%, rgba(0, 80, 120, 0.7) 100%);
                padding: 30px;
                margin-bottom: 30px;
                text-align: center;
            ...
            .section {{
                background: linear-gradient(135deg, rgba(0, 30, 60, 0.8) 0%, rgba(0, 60, 90, 0.6) 100%);
                padding: 25px;
                margin-bottom: 25px;
            ...
            .performance-card {{
                background: linear-gradient(135deg, rgba(0, 40, 80, 0.8) 0%, rgba(0, 80, 120, 0.6) 100%);
                padding: 20px;
                border: 1px solid #0088ff;
                position: relative;
                overflow: hidden;
            ...
            /* Matrix rain effect - Cyan version */
            .matrix-rain {{
                position: fixed;
                top: 0;
                left: 0;
            ...
            ::-webkit-scrollbar-thumb:hover {{
                background: linear-gradient(180deg, #00ffff 0%, #0055ff 100%);
            }}
    ...
    <body>
        ...
        <div class="container">
            <div class="header">
                <h1>J.A.R.V.I.S. SYSTEM DIAGNOSTICS</h1>
                <div class="creator">SABARI425 ORGANIZATION</div>
                <p>> PROTOCOL INITIATED: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
                <p>> TARGET SYSTEM: {socket.gethostname()} | ARCHITECTURE: {platform.platform()}</p>
    ...
    for section_name, data in sections_data.items():
        ...
        if data:
            ...
            html_content += '<p style="color: #0088ff; text-align: center; padding: 30px; font-style: italic;">> NO DATA AVAILABLE FOR ANALYSIS</p>'

        html_content += '</div>'

    html_content += """
            <div class="footer">
                <p>> DIAGNOSTIC PROTOCOL COMPLETED: """ + datetime.now().strftime('%H:%M:%S') + """</p>
                <p>> J.A.R.V.I.S. v3.0 | SABARI425 ORGANIZATION | SECURITY LEVEL: MAXIMUM</p>
                <p style="margin-top: 10px; color: #00ffff; font-size: 0.8em;">
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                tables.forEach(table => {
                    ...
                    headers.forEach((header, index) => {
                        ...
                        header.title = 'Click to sort analysis data';
                ...
                // Add HUD element animations
                const hudElements = document.querySelectorAll('.hud-corner');
                hudElements.forEach((element, index) => {
                    setInterval(() => {
                        element.style.boxShadow = '0 0 20px #00ffff';
                        setTimeout(() => {
                            element.style.boxShadow = '0 0 30px #0088ff';
                        }, 1000);
                    }, 2000 + index * 500);
                });
            });
    ...
    print("\n\n")
    print_status("JARVIS diagnostic report compiled successfully!", "SUCCESS")
...
# -------------------------------------------------------------------
#  MAIN EXECUTION - JARVIS CYAN THEME
# -------------------------------------------------------------------
if __name__ == "__main__":
    print()
    print_colored("J.A.R.V.I.S. DIAGNOSTIC PROTOCOL INITIATED...", Colors.BRIGHT_CYAN)
    print_status("Initializing all diagnostic modules...", "INFO")
    ...
    try:
        ...
        print()
        print_colored("▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄▄", Colors.CYBER_CYAN)
        print_colored("█                                                       █", Colors.CYBER_CYAN)
        print_colored("█           DIAGNOSTIC PROTOCOL COMPLETE              █", Colors.CYBER_CYAN)
        print_colored("█                                                       █", Colors.CYBER_CYAN)
        print_colored("▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀▀", Colors.CYBER_CYAN)
        print("\n\n\n")
        ...
        print(); print()
        print_status("DIAGNOSTIC REPORT SUMMARY:", "SUCCESS")
        print_status(f"Output File: {html_path}", "DATA")

        if health_score >= 80:
            print("\t", end=''); print_status(f"System Integrity: {health_score}/100 (OPTIMAL)", "SUCCESS")
        elif health_score >= 60:
            print("\t", end=''); print_status(f"System Integrity: {health_score}/100 (STABLE)", "WARNING")
        else:
            print("\t", end=''); print_status(f"System Integrity: {health_score}/100 (CRITICAL)", "ERROR")

        print("\n\n")

        print_status("DIAGNOSTIC MODULES EXECUTED:", "INFO")
        sections = [
        ...
        for section in sections:
            print_colored(f"    ◆ {section}", Colors.CYAN)

        print("\n\n\n\n\n\n", end='')
        print_status("Launching diagnostic interface...", "INFO")
        ...
        if not browser_opened:
            try:
                ...
                print_status("JARVIS interface launched successfully", "SUCCESS")
                browser_opened = True
            except Exception as e:
        ...
        print()
        print_status("JARVIS diagnostic protocol completed successfully!", "SUCCESS")
        print_status("All systems operational. Have a nice day, Sir.", "INFO")

    except Exception as e:
        print_status(f"DIAGNOSTIC PROTOCOL FAILED: {str(e)}", "ERROR")
        print_status("Please verify system permissions and try again", "WARNING")
